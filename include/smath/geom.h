...
namespace slib {
	namespace sgeom {
		...
		extern inline svec2d<T> midpoint(const v2i& p1, const v2i& p2) { return svec2d<T>((T)(p1.x + p2.x) / 2, (T)(p1.y + p2.y) / 2); }
		extern inline v2f midpoint(const v2f& p1, const v2f& p2) { return v2f(0.5f * (p1.x + p2.x), 0.5f * (p1.y + p2.y)); }
		extern inline v2d midpoint(const v2d& p1, const v2d& p2) { return v2d(0.5 * (p1.x + p2.x), 0.5 * (p1.y + p2.y)); }
		...
		extern inline v2d scale(const v2d& p, const v2d& ratio) { return v2d(p.x * ratio.x, p.y * ratio.y); }
		...
		extern inline v2f skew(const v2f& p, const float& ratio, smath::DIRECTION dir) {
			if (dir == smath::HORIZONTAL) return v2f(p.x + p.y * ratio, p.y);
			else return v2f(p.x, p.x * ratio + p.y);
		}
		extern inline v2d skew(const v2d& p, const double& ratio, smath::DIRECTION dir) {
			if (dir == smath::HORIZONTAL) return v2d(p.x + p.y * ratio, p.y);
			else return v2d(p.x, p.x * ratio + p.y);
		}
		...
		extern inline double azimuth(const v3i& p) { return argument(p.projection(smath::XY_PLANE)); }
		extern inline double azimuth(const v3f& p) { return argument(p.projection(smath::XY_PLANE)); }
		extern inline double azimuth(const v3d& p) { return argument(p.projection(smath::XY_PLANE)); }
		extern inline double altitude(const v3i& p) { return (p.z < 0.0 ? -1.0 : 1.0) * acos(length(p.projection(smath::XY_PLANE)) / length(p)); }
		extern inline double altitude(const v3f& p) { return (p.z < 0.0 ? -1.0 : 1.0) * acos(length(p.projection(smath::XY_PLANE)) / length(p)); }
		extern inline double altitude(const v3d& p) { return (p.z < 0.0 ? -1.0 : 1.0) * acos(length(p.projection(smath::XY_PLANE)) / length(p)); }
		...
		template<typename T>
		extern inline svec2d<T> center(const sarea& area) {
			return svec2d<T>((T)area.ori_x + (T)area.width / 2, (T)area.ori_y + (T)area.height / 2);
		}
		extern inline v2f center(const sareaf& area) {
			return v2f(area.ori_x + 0.5f * area.width, area.ori_y + 0.5f * area.height);
		}
		extern inline v2d center(const saread& area) {
			return v2d(area.ori_x + 0.5 * area.width, area.ori_y + 0.5 * area.height);
		}
		extern inline double length(const sveci& p) {
			double len = 0.0; sforeach(p) { len += (double)E_ * E_; } return sqrt(len);
		}
		...
		extern inline double distance(const sveci& p1, const sveci& p2) {
			if (p1.size() != p2.size())
				throw smath::SMathException(ERR_INFO, smath::DIMENSION_SIZE_ERR, "p1, p2", DIMENSION_SIZE_DIFF(p1.size(), p2.size()));
			return length(p2 - p1);
		}
