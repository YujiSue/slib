...
namespace slib {
    ...
    template<typename T>
	inline T Range<T>::length(bool closed) const { return end-begin+(closed?1:0); }
    template<typename T>
	inline bool Range<T>::include(const T &val) const { return (begin <= val && val <= end)?true:false; }
    template<typename T>
	inline bool Range<T>::include(const Range<T> &rng) const { return begin <= rng.begin && rng.end <= end; }
    template<typename T>
	inline bool Range<T>::overlap(const Range<T> &rng) const { return begin <= rng.end && rng.begin <= end; }
    template<typename T>
	inline void Range<T>::shift(const T &s) { begin += s; end += s; }
    template<typename T>
	inline void Range<T>::expand(const T &e) { end += e; }
    template<typename T>
	inline void Range<T>::merge(const Range &rng) {
        if(rng.begin < begin) begin = rng.begin;
        if(end < rng.end) end = rng.end;
    }
    template<typename T>
	inline void Range<T>::difference(const Range &rng) {
        if (!overlap(rng) || include(rng)) return;
        if(include(rng.begin)) end = rng.begin-1;
        else if (include(rng.end)) begin = rng.end+1;
    }
    template<typename T>
	inline void Range<T>::conjunction(const Range<T> &rng) {
    ...
    template<typename T>
	inline Range<T> Range<T>::shifted(const T &s) const { return Range(begin+s, end+s); }
    template<typename T>
	inline Range<T> Range<T>::expanded(const T &e) const { return Range(begin, end+e); }
    template<typename T>
	inline Range<T> Range<T>::merged(const Range<T> &rng) const {
        Range rng_ = *this;
    ...
    template<typename T>
	inline Range<T> Range<T>::differenced(const Range &rng) const {
        Range<T> r(*this);
        r.difference(rng);
        return r;
    }
    template<typename T>
	inline Range<T> Range<T>::conjunctioned(const Range<T> &rng) const {
        Range rng_ = *this;
    ...
    template<typename T>
	inline bool Range<T>::operator < (const T &pos) const { return end < pos; }
    template<typename T>
	inline bool Range<T>::operator < (const Range<T> &rng) const {
