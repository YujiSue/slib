...
sushort slib::smedia::SStroke::toLineStyle(const char *style) {
    String st = style;
    if (st == "solid") return slib::sstyle::SOLID_LINE;
    if (st == "broken") return slib::sstyle::BROKEN_LINE;
    if (st == "double") return slib::sstyle::DOUBLE_LINE;
    return slib::sstyle::STROKE_NONE;
}

SStroke::SStroke() : type(slib::sstyle::SOLID_LINE), width(1.0), color(color::BLACK) {}
...
sushort slib::smedia::SBrush::toBrushStyle(const String &style) {
    if (style == "fill") return slib::sstyle::FILL_UNIFORM;
    if (style == "line_grad") return slib::sstyle::LINEAR_GRAD;
    if (style == "radgrad") return slib::sstyle::RADIAL_GRAD;
    return slib::sstyle::FILL_NONE;
}

SBrush::SBrush() : type(slib::sstyle::FILL_NONE) {}
SBrush::SBrush(sushort t, const SColor &col) : type(t), color(col) {}
SBrush::SBrush(const SColor & col) {
	type = slib::sstyle::FILL_UNIFORM;
	color = col;
}
...
bool SBrush::isGradient() const { return type&slib::sstyle::FILL_GRADIENT; }
