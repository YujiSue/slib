#include "sscience/numeric.h"

using namespace slib;
using namespace slib::smath;

double slib::ssci::legendre(double d, size_t n) {
	if (!n) return 1.0;
	if (n == 1) return d;
...
double slib::ssci::laguerre(double d, size_t n) {
	if (!n) return 1.0;
	if (n == 1) return 1.0 - d;
...
svdvec slib::ssci::GLGD_QUADR_WX = {
...
svdvec slib::ssci::GLGR_QUADR_WX = {
...
double slib::ssci::gaussIntegral(std::function<double(double)>& func, const svecd& vec) {
	double val = 0.0;
	sforeach(vec) { val += E_ * func(E_NEXT); NEXT_; }
	return val;
}
double slib::ssci::glgdIntegral(std::function<double(double)>& func, int n, sranged range) {
	if (n == 1) return 0.0;
	if (2 <= n && n <= 7) {
...
double slib::ssci::glgrIntegral(std::function<double(double)>& func, int n) {
	if (n == 1) return 0.0;
	if (2 <= n && n <= 7)
...
double slib::ssci::mcIntegral(std::function<double(double)>& func, sranged border, sranged range, int repeat) {
	SRandom rand1, rand2;
	int count = 0;
	sforin(i, 0, repeat) {
