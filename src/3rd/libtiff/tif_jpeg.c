...
static const TIFFField jpegFields[] = {
    { TIFFTAG_JPEGTABLES, -3, -3, TIFF_UNDEFINED, 0, TIFF_SETGET_C32_UINT8, TIFF_SETGET_C32_UINT8, FIELD_JPEGTABLES, FALSE, TRUE, (char*)"JPEGTables", NULL },
    { TIFFTAG_JPEGQUALITY, 0, 0, TIFF_ANY, 0, TIFF_SETGET_INT, TIFF_SETGET_UNDEFINED, FIELD_PSEUDO, TRUE, FALSE, (char*)"", NULL },
    { TIFFTAG_JPEGCOLORMODE, 0, 0, TIFF_ANY, 0, TIFF_SETGET_INT, TIFF_SETGET_UNDEFINED, FIELD_PSEUDO, FALSE, FALSE, (char*)"", NULL },
    { TIFFTAG_JPEGTABLESMODE, 0, 0, TIFF_ANY, 0, TIFF_SETGET_INT, TIFF_SETGET_UNDEFINED, FIELD_PSEUDO, FALSE, FALSE, (char*)"", NULL }
};
...
static int
TIFFjpeg_set_quality(JPEGState* sp, int quality, jboolean force_baseline)
{
	return CALLVJPEG(sp,
...
static int
TIFFjpeg_suppress_tables(JPEGState* sp, jboolean suppress)
{
	return CALLVJPEG(sp, jpeg_suppress_tables(&sp->cinfo.c, suppress));
}

static int
TIFFjpeg_start_compress(JPEGState* sp, jboolean write_all_tables)
{
	return CALLVJPEG(sp,
...
static int
TIFFjpeg_read_header(JPEGState* sp, jboolean require_image)
{
...
TIFFjpeg_data_dest(JPEGState* sp, TIFF* tif)
{
	...
	sp->dest.init_destination = (void(*)(j_compress_ptr))std_init_destination;
	sp->dest.empty_output_buffer = (jboolean(*)(j_compress_ptr))std_empty_output_buffer;
	sp->dest.term_destination = (void(*)(j_compress_ptr))std_term_destination;
}
...
JPEGFixupTagsSubsamplingReadByte(struct JPEGFixupTagsSubsamplingData* data, uint8* result)
{
	if (data->bufferbytesleft==0)
	{
		...
		if (TIFFReadFile(data->tif,data->buffer,(tmsize_t)m)!=(tmsize_t)m)
			return(0);
		data->buffercurrentbyte=(uint8*)data->buffer;
